class Applicant < ActiveRecord::Base
  ...
  state_machine :initial => :applied do
    ...
    event :recommendation_recieved do
      transition :submitted => :complete, :if => lambda { |applicant|
        applicant.submitted? && applicant.recommendations.select { |rec| rec.received?}.size > 1
      }
    end
  ...
  def for_admin_html
    ...
    <h4>Resume:</h4>
    <a href='#{self.resume.url.gsub(/submitteds|applicantss|rejecteds|completes|accepteds|applieds/, 'applicants')}'>Download Resume</a><br />
