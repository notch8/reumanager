class Applicant < ActiveRecord::Base
  ...
  def for_admin_html
    ...
    <b>Are you a first generation college student?</b> #{self.try(:student)}<br>
    <b>How did you hear about us?</b> #{self.try(:found_us)}<br>
    <b>Permission to share?</b> #{self.try(:permission_to_share)}<br>

    <h4>Statement:</h4>
    #{self.try(:statement)}<br>

    <h4>Research Interests:</h4>
    <b>Mentor 1:</b> #{self.try(:mentor1)}<br>
    <b>Mentor 2:</b> #{self.try(:mentor2)}<br>
    <b>Mentor 3:</b> #{self.try(:mentor3)}<br>
